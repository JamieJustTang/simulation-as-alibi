...
Coordination is the field's proving ground — the question the entire LLM-agent-society program inherits from the generative-agents lineage of Park et al.'s Smallville: can populations of language models, without central orchestration, converge on shared plans, team structures, and collective action? The 31 papers here split along a visible fault line. On one side stand the strongest spontaneous-emergence claims. *Shall We Team Up* deliberately removes instruction-shaping to show agents "spontaneously learning to cooperate in the wild"; *AI agents can coordinate beyond human scale* finds LLMs "spontaneously form cohesive groups" governed by a "majority force coefficient" and a critical group size; *Emergent Relational Order in LLM Agent Societies* reports agents "spontaneously reproducing" guanxi-based ethics and authority stratification. Each narrates coordination as arising from the agents themselves, with the designer's scaffolding either removed or relegated to background. On the other side sit papers that return to the designer: scaffolded collaboration, topology-aware simulation, and policy platforms such as CoRenew attribute coordination explicitly to authored mechanisms, treating the interactive environment — not the model's intrinsic properties — as the decisive variable.

Between these poles lie the collusion studies, which reframe coordination itself as a safety problem. *Colosseum* and *Evaluating LLM Agent Collusion in Double Auctions* treat the same self-organization prized in one line of work as the covert coalition to be audited in another, isolating the conditions — communication channels, oversight pressure, network topology — under which collusion "emerges." The framing inversion is instructive: the very property that makes coordination celebrated in the spontaneous-emergence literature becomes an object of suspicion the moment a designer acknowledges having built the channel through which it forms.

A substantial middle band is occupied by framework and benchmark papers. *AgentVerse* and *Agentopia* build platforms that "explore" emergent social behaviors; the CRSEC norm architecture (*Emergence of Social Norms in Generative Agent Societies*) explicitly engineers creation, spreading, evaluation, and compliance modules to "empower the emergence" of norms. These are the papers most likely to code DV=Partial: the architecture is described in detail, yet the explanation sentence still hands the outcome to the agents ("conversations and thoughts drive the emergence"). The design is present everywhere except in the causal claim.

A fourth cluster documents where coordination fails, and in doing so makes the designer's role legible by its absence. *Multi-Agent LLMs Fail to Explore Each Other* shows agents lapse into myopic, polarized interaction patterns unless an explicit exploration scaffold (MACE) is added; *AI Agents Alone Are Not (Yet) Sufficient for Social Simulation* argues population dynamics are mediated by environment co-dynamics, scheduling, and information priors rather than agent–agent messaging; the *Ethical Coordination of LLM Multi-Agent Systems* jury runs end in hung verdicts because anchoring defeats persuasion. *The Hunger Game Debate* reaches the complementary conclusion that the "explicit design of the interactive environment" — not the intrinsic properties of the models — is the critical factor shaping multi-agent dynamics.

The applied tail points toward deployment. Education scaffolds ("Deep Think before Speak"), POSIM's public-opinion governance simulation, CoRenew's multifamily-redevelopment policy platform, LiveCultureBench's multi-cultural town, and SpeechAgents' multimodal communication all treat coordination as something to be engineered for a purpose, and accordingly code DV=Full. The audit distribution is the corpus in miniature: 5 Absent, 12 Partial, and 14 Full DV across 13 High-, 14 Medium-, and 4 Low-EI papers. The High-EI papers cluster at the Absent end — cohesion is "spontaneous," a "majority force coefficient," an emergent property of the group — while the applied and benchmark papers cluster at the Full end. The same interaction topologies, action grammars, and memory modules condition both; the difference is whether the explanation sentence names them.
